...
func (t *Transformer) addToBatch(ctx context.Context, e *entry.Entry, source string, matches bool) {
	...
	t.Logger().Debug("Batch state after adding entry",
		...
		zap.String("combined_content_preview", truncateString(batch.recombined.String(), 100)))

	// Check if any size limits are reached
	hitMaxLogSize := t.maxLogSize > 0 && int64(batch.recombined.Len()) > t.maxLogSize
	hitMaxBatchSize := t.maxBatchSize > 0 && batch.numEntries >= t.maxBatchSize
	hitMaxUnmatchedBatchSize := !batch.matchDetected && t.maxUnmatchedBatchSize > 0 && batch.numEntries >= t.maxUnmatchedBatchSize

	if hitMaxLogSize || hitMaxBatchSize || hitMaxUnmatchedBatchSize {
		t.Logger().Debug("Size limit reached, triggering flush",
			zap.String("source", source),
			zap.Bool("hit_max_log_size", hitMaxLogSize),
			zap.Bool("hit_max_batch_size", hitMaxBatchSize),
			zap.Bool("hit_max_unmatched_batch_size", hitMaxUnmatchedBatchSize),
			zap.Int64("max_log_size", t.maxLogSize),
			zap.Int("max_batch_size", t.maxBatchSize),
			zap.Int("max_unmatched_batch_size", t.maxUnmatchedBatchSize),
			zap.Int("current_buffer_size", batch.recombined.Len()),
